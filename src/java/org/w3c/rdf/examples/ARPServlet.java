/*******************************************************************************
 * 
 * ARPServlet - this servlet implements an RDF Validation service. As of this
 * writing, the following RDF validation service used this servlet:
 * 
 * http://www.w3.org/RDF/Validator/
 * 
 * **********************************************************************
 * 
 * Copyright &copy; World Wide Web Consortium, (Massachusetts Institute of
 ...
 * **********************************************************************
 * 
 * This servlet is a wrapper for the ARP RDF parser. See the following for
 * information about the ARP RDF parser:
 * 
 ...
 * 
 * o This servlet supports the HTTP POST operation; it does not support the HTTP
 * GET operation
 * 
 * o Depending upon the parameters given to the servlet it may invoke a GraphViz
 * suprocess to generate a graph of the RDF. See the following for more
 ...
 * 
 * o Depending upon the parameters given to the servlet, the RDF to be validated
 * may be copied to a file. The name of the file is automatically generated via
 * Java's temporary file APIs. The location of the directory where the file is
 * stored is configured via the serverlet's init() method. See below for more
 * information.
 * 
 ...
 * **********************************************************************
 * 
 * HTTP POST parameters - the servlet expects/assumes the following variables
 * are defined via the HTTP POST request:
 * 
 * RDF - the RDF (assumed to be in RDF/XML syntax) to be validated
 * 
 * SAVE_DOT_FILE - if "on", the GraphViz DOT file is saved and a link to the
 * file is returned; otherwise the DOT file is not saved
 * 
 * SAVE_RDF - if "on", the RDF will be copied to a file; otherwise the RDF is
 * not copied to a file
 * 
 * EMBEDDED_RDF - if "on", then the RDF is not enclosed in <RDF>... </RDF> tags;
 * otherwise it assumed that the RDF is enclosed in these tags.
 * 
 * URI - the URI of the RDF to validate
 * 
 * PARSE - if "Parse RDF", then parse RDF from the textarea; else download from
 * URI; if not present, prefer URI, but if URI is empty, parse RDF (old
 * behavior).
 * 
 * ORIENTATION - the graph's orientation (left to right or top to bottom);
 * default is left to right
 * 
 * FONT_SIZE - the font size to use (10, 12, 14, 16 and 20 are supported); the
 * default is 10
 * 
 ...
 * 
 * PRINT_BOTH - display triples and a graph (the default) PRINT_TRIPLES - only
 * display the triples PRINT_GRAPH - only display the graph
 * 
 * FORMAT - the graph's output format. Supported values are:
 * 
 * GIF_EMBED - embed the graph as a GIF GIF_LINK - don't embed the GIF but
 * create a link for it SVG_LINK - create the graph in SVG format and create a
 * link to the file SVG_EMBED - create the graph in SVG format and embed it in
 * an object tag ISV_ZVTM - IsaViz/ZVTM (Dynamic View - requires Java Plug-in
 * 1.4) PNG_EMBED - create the graph in PNG format and embed the graph in the
 * document that is returned (the default) PNG_LINK - create the graph in PNG
 * format and create a link to the file PS_LINK - create a PostScript image of
 * the file and a link to the file HP_PCL_LINK - create a HPGL/2 - PCL
 * (Laserwriter) image of the file and a link to the file HP_GL_LINK - create a
 * HPGL - PCL (pen plotter) image of the file and a link to the file
 * 
 * NTRIPLES if "on" the tabular output will be in the NTriples format; otherwise
 * a table of Subject, Predicate, Objects will be generated
 * 
 * **********************************************************************
 * 
 * Server Initialization - this servlet requires the following parameters be set
 * in the servlet's init() method - via the ServletConfig object:
 * 
 * GRAPH_VIZ_ROOT - the absolute path of the top-level directory containing
 * GraphViz's binary distribution
 * 
 * GRAPH_VIZ_PATH - absolute or relative (based on GRAPH_VIZ_ROOT) path of the
 * DOT executable (e.g. dotneato/dot) - the program used to generate a graph
 * from a DOT file.
 * 
 * GRAPH_VIZ_FONT_DIR - absolute or relative (based on GRAPH_VIZ_ROOT) path of
 * the fonts directory used by GraphViz (e.g. Fonts)
 * 
 * SERVLET_TMP_DIR - the absolute path of the directory to be used to store
 * temporary files used by the servlet and GraphViz. This directory must be
 * writable by the servlet.
 * 
 * NOTE - Some files created by the servlet are not removed by servlet (e.g.
 * graph image files).
 * 
 * If any of these parameters are not defined, the servlet will NOT validate the
 * RDF.
 * 
 * **********************************************************************
 * 
 * Dependencies - this servlet requires the following Java packages as well as
 * GraphViz (described above):
 * 
 ...
 * **********************************************************************
 * 
 * Author: Art Barstow <barstow@w3.org>Author (internationalization): Martin J.
 * Duerst <duerst@w3.org>Author (maintenance): Emmanuel Pietriga
 * <emmanuel@w3.org>
 * 
 * $Id: ARPServlet.java,v 1.1 2005/05/16 22:19:30 pangloss Exp $
 *
 ******************************************************************************/
...
public class ARPServlet extends HttpServlet {
    final static public String REVISION = "$Id: ARPServlet.java,v 1.1 2005/05/16 22:19:30 pangloss Exp $";
    ...
    private static final String DEFAULT_TTF_FONT = "arialuni"; //was 'cyberbit'
    private static final String DEFAULT_FONT = "Courier"; //could changed to
                                                          // 'arialuni' to get
                                                          // coverage for
                                                          // japanese/russian/...
    ...
    //used to detect whether the provided document contains at least one
    // triple, or if it is not RDF at all
    //necessary because ARP does not report any error when parsing an XML
    // document which does not contain any
    //RDF statement
     ...
     * 
     * @param directory the file's directory @param prefix the file's prefix
     * name (not its directory) @param suffix the file's suffix or extension
     * name @return a File object if a temporary file is created; null otherwise
     */
     ...
     * the String
     * 
     * @param uri the URI to open @return the content at the URI or null if any
     * error occurs
     */
    private String getRDFfromURI(String uri) throws getRDFException {
        /*
         * add something like this code here, to allow reading from a file: (if
         * we really want to allow this!) File ff = new File(uri); in = new
         * FileInputStream(ff);
         */
        URL url = null;
        try {
        ...
        try {
            ...
            //      if (APPFcharset!=null){xmlstart = new
            // String(s.substring(ignoreBytes).getBytes("iso-8859-1"),
            // APPFcharset);}
            //      else {xmlstart=new
            // String(s.substring(ignoreBytes).getBytes("iso-8859-1"));APPFcharset
            // = "UTF-8";}
            xmlstart = new String(s.substring(ignoreBytes).getBytes(
                    "iso-8859-1"), APPFcharset);
        ...
        try {// read whole file as characters
            ...
            throw new getRDFException(
                    "Undecodable data when reading URI at byte "
                            + bytenum
                            + " using encoding '"
                            + finalCharset
                            + "'."
                            + " Please check encoding and encoding declaration of your document.");
        }
    ...
    /*
     * Copy the given string of RDF to a file in the given directory. This is
     * only done if the servlet is explictly asked to save the RDF to a file.
     * 
     * @param tmpDir the file's directory @param rdf the string of RDF
     */
    private void copyRDFStringToFile(String tmpDir, String rdf) {
        try {
            ...
            System.err.println(SERVLET_NAME
                    + ": error occured trying to save RDF to file '" + tmpDir
                    + TMP_FILE_PREFIX + SUFFIX_RDF + "'.");
            return;
        }
    }

    /*
     * Given the graph's format option, return either the corresponding command
     * line option for that option or the file name suffix for the graph option.
     * For example GIF files have ".gif" for its suffix and GraphViz uses
     * "-Tgif" for the command line.
     * 
     * NOTE: default is PNG.
     * 
     * @param graphFormat the graph's output format @param suffix. If true, the
     * name returned is for the graph's file name suffix; otherwise, the name
     * returned is for the graph's command line option. @return the suffix to
     * use for the graph's output file
     */
    private String getFormatName(String graphFormat, boolean suffix) {

        String name = (suffix) ? "." : "-T";

        if (graphFormat.equals(FORMAT_PNG_EMBED))
            return name + NAME_PNG;
        if (graphFormat.equals(FORMAT_PNG_LINK))
            return name + NAME_PNG;
        if (graphFormat.equals(FORMAT_GIF_EMBED))
            return name + NAME_GIF;
        if (graphFormat.equals(FORMAT_GIF_LINK))
            return name + NAME_GIF;
        if (graphFormat.equals(FORMAT_SVG_LINK))
            return name + NAME_SVG;
        if (graphFormat.equals(FORMAT_SVG_EMBED))
            return name + NAME_SVG;
        if (graphFormat.equals(FORMAT_ISV_ZVTM))
            return name + NAME_SVG;
        if (graphFormat.equals(FORMAT_PS_LINK))
            return name + NAME_PS;
        if (graphFormat.equals(FORMAT_HP_GL_LINK))
            return name + NAME_HPGL;
        if (graphFormat.equals(FORMAT_HP_PCL_LINK))
            return name + NAME_PCL;

        return name + NAME_PNG;
    }

    /*
     * Invokes the GraphVis program to create a graph image from the the given
     * DOT data file
     * 
     * @param dotFileName the name of the DOT data file @param outputFileName
     * the name of the output data file @param graphFormat the graph's format
    ...
    private boolean generateGraphFile(String dotFileName,
        ...
        String cmdArray[] = { m_GraphVizPath, formatOption, "-o",
    ...
    /*
     * Returns a parameter from a request or the parameter's default value.
     * 
     * @param req a Servlet request @param param the name of the parameter
     * @param defString the string returned if the param is not found @return if
     * the request contains the specfied parameter its value in the request is
     * returned; otherwise its default value is returned
     */
    ...
    /*
     * If the request contains any graph-related parameters, pass them to the
     * graph consumer for handling
     * 
     * @param req the response @param pw the PrintWriter @param consumer the
     * GraphViz consumer @param bitmap true=generate a bitmap (GIF or PNG),
     * false=generate PostScript, SVG, etc.
     */
    private void processGraphParameters(HttpServletRequest req, PrintWriter pw,
        ...
        pw.println("node [fontname=\""
                + ((bitmap) ? DEFAULT_TTF_FONT : DEFAULT_FONT) + "\",fontsize="
                + fontSize + ",color=" + nodeColor + ",fontcolor="
                + nodeTextColor + "];");

        // Add an attribute for all of the graph's edges
        pw.println("edge [fontname=\""
                + ((bitmap) ? DEFAULT_TTF_FONT : DEFAULT_FONT) + "\",fontsize="
                + fontSize + ",color=" + edgeColor + ",fontcolor="
                + edgeTextColor + "];");
    ...
    private static class SaxErrorHandler implements org.xml.sax.ErrorHandler {
         ...
         * 
         * @param out the servlet's PrintWriter @param silent if false, output
         * is suprressed
         */
         ...
         * 
         * @param e the SAX Parse Exception @return a formatted string
         */
        private static String format(org.xml.sax.SAXParseException e) {
            ...
            msg = replaceString(msg, "\"", "&quot;");
            msg = replaceString(msg, "'", "&apos;");
            return msg + "[Line = " + e.getLineNumber() + ", Column = "
                    + e.getColumnNumber() + "]";
        }
        ...
        public void error(org.xml.sax.SAXParseException e)
            ...
            if (e instanceof com.hp.hpl.jena.rdf.arp.ParseException) {
                ...
                //      if
                // (pe.getErrorNumber()==com.hp.hpl.jena.rdf.arp.ARP.WARN_NOT_SUPPORTED
                // && pe.getMessage().indexOf("datatyping")!=-1){
        ...
        public void warning(org.xml.sax.SAXParseException e)
            ...
            if (e instanceof com.hp.hpl.jena.rdf.arp.ParseException) {
                ...
                // pe=(com.hp.hpl.jena.rdf.arp.ParseException)e;
                //      if
                // (pe.getErrorNumber()==com.hp.hpl.jena.rdf.arp.ARP.WARN_NOT_SUPPORTED
                // && pe.getMessage().indexOf("datatyping")!=-1){
                //          datatypeErrors+=String.valueOf(pe.getLineNumber())+", ";
                ...
                this.errors += "Warning: " + format(e) + "<br />";
            }
        }
         ...
         * 
         * @return the error messages or an empty string if there are no
         * messages
         */
         ...
         * 
         * @return the fatal error messages or an empty string if there are no
         * messages
         */
         ...
         * 
         * @return the warning messages or an empty string if there are no
         * messages
         */
     ...
     * 
     * @param out the servlet's output Writer @param pw the graph file's
     * PrintWriter @param dotFile the File handle for the graph file @param rdf
     * the RDF text @param req a Servlet request @param graphFormat the graph's
     * format @param saveRDF the RDF can be cached [saved to the file system]
     * @param saveDOTFile the DOT file should be cached
     */
    private void generateGraph(PrintWriter out, PrintWriter pw, File dotFile,
            String rdf, HttpServletRequest req, String graphFormat,
        ...
        try {
            ...
            if (outputFile == null) {
                out
                        .println("Failed to create a temporary file for the graph. A graph cannot be generated.");
                dotFile.delete();
                return;
            }
            ...
            if (saveDOTFile) {
                ...
                String dotPath = SERVLET_NAME + SUFFIX_TMP_DIR + File.separator
                        + dotFile.getName();
                out.println("<a href=\"" + dotPath
                        + "\">Download the DOT file.</a><br /><br />");
            } else {
                // Delete it ...
                dotFile.delete();
            }
            ...
            if (graphFormat.equals(FORMAT_GIF_EMBED)
                ...
                if (outputFile.length() > 0) {
                    out
                            .println("<object type=\"image/svg+xml\" name=\"rdfsvg\" data=\"http://www.w3.org/RDF/Validator/"
                                    + imagePath
                                    + "\" width=\"640\" height=\"480\">Your browser does not support the &lt;object&gt; tag. The SVG representation of the model cannot be embedded in this page. You can use <b>SVG - link</b> or update your browser to a version supporting the &lt;object&gt; tag.</object>");
                } else
                    out.println("The graph image file is empty.");
            } else if (graphFormat.equals(FORMAT_ISV_ZVTM)) {
                if (outputFile.length() > 0) {
                    out
                            .println("<applet code=\"org.w3c.IsaViz.applet.IsvBrowser.class\"");
                    out
                            .println("archive=\"lib/zvtm.jar,lib/isvapp.jar,lib/xercesImpl.jar,lib/xmlParserAPIs.jar\"");
                    out.println("width=\"640\" height=\"480\">");
                    out
                            .println("<param name=\"type\" value=\"application/x-java-applet;version=1.4\" />");
                    out
                            .println("<param name=\"scriptable\" value=\"false\" />");
                    out.println("<param name=\"width\" value=\"640\" />");
                    out.println("<param name=\"height\" value=\"480\" />");
                    out
                            .println("<param name=\"svgFile\" value=\"http://www.w3.org/RDF/Validator/"
                                    + imagePath + "\" />");
                    out.println("</applet>");
                } else
                    out.println("The graph image file is empty.");
    ...
    /*
     * Search the given string for substring "key" and if it is found, replace
     * it with string "replacement"
     * 
     * @param input the input string @param key the string to search for @param
     * replacement the string to replace all occurences of "key" @return if no
     * substitutions are done, input is returned; otherwise a new string is
     * returned.
     */
        ...
        try {
            ...
            out
                    .println("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\"\n"
                            ...
                            + "<html><head>\n"
                            + "<title>RDF Validator Results</title>\n"
                            ...
                            + "src='http://www.w3.org/Icons/WWW/w3c_home' alt='W3C' border='0'\n"
                            + "    height='48' width='72' /></a>\n"
                            + "RDF Validation Results\n"
                            +
                            //           "<p>classpath="+System.getProperty("java.class.path")+"</p>"+
                            "<a href='http://www.w3.org/RDF/'\n"
                            + "    title='RDF Resource Description Framework'><img border='0'\n"
                            + "    src='http://www.w3.org/RDF/icons/rdf_powered_button.48'\n"
                            + "    alt='RDF Resource Description Framework Powered Icon' /></a></h1>\n"
                            + "<p><a href='#source'>Source</a> | "
                            + "<a href='#triples'>Triples</a> | "
                            + "<a href='#messages'>Messages</a> | "
                            + "<a href='#graph'>Graph</a> | "
                            + "<a href='#feedback'>Feedback</a> | "
                            + "<a href='http://www.w3.org/RDF/Validator/'>Back to Validator Input</a></p>");

        } catch (Exception e) {
            System.err.println("Exception (printDocumentHeader): "
                    + e.getMessage());
        }
    }
     ...
     * 
     * @param out the servlet's output Writer @param rdf the RDF code @param
     * needCR if true, add a CarriageReturn to the output; if false, do not add
     * it
     */
    private void printListing(PrintWriter out, String rdf, boolean needCR) {
        try {
            out.println("<hr title='original source' />"
                    + "<h3><a name='source' id='source'>"
                    + "The original RDF/XML document</a></h3>" + "<pre>");
            ...
            do {
                ...
                out.print("<a name=\"" + lineNum + "\">" + lineNum + "</a>: "
                        + tok + terminator);
                lineNum++;
            } while (nl != -1);
     ...
     * 
     * @param out the servlet's output Writer @param nTriples if true, output is
     * N-Triples syntax
     */
    private void printTripleTableHeader(PrintWriter out, boolean nTriples) {
        try {
            if (nTriples) {
                out
                        .println("<h3><a name='triples' id='triples'>"
                ...
                out.println("<table frame='border' rules='all'><tr>"
                        + "<td><b>Number</b></td>" + "<td><b>Subject</b></td>"
                        + "<td><b>Predicate</b></td>"
                        + "<td><b>Object</b></td>" + "</tr>");
            }
        } catch (Exception e) {
            System.err.println("Exception (printTripleTableHeader): "
                    + e.getMessage());
        }
    }
     ...
     * N-Triples syntax
     */
    private void printTripleTableFooter(PrintWriter out, boolean nTriples) {
        try {
            if (nTriples)
                out.println("</pre>");
            else
            ...
            System.err.println("Exception (printTripleTableFooter): "
                    + e.getMessage());
        }
    }
     ...
     * 
     * @param out the servlet's output Writer @param rdf the RDF code
     */
    private void printDocumentFooter(PrintWriter out, String rdf) {
        try {
            ...
            out
                    .println("<input type='hidden' name='RDF' value=\"&lt;?xml version=&quot;1.0&quot;&gt;");
            ...
            System.err.println("Exception (printDocumentFooter): "
                    + e.getMessage());
        }
    }
     ...
     * Create a formatted string from the exception's message
     * 
     * @param e any exception other than a SAXParseException @return a formatted
     * string
     */
    ...
    public String getServletInfo() {
        return "Servlet wrapper for the ARP RDF parser. This is revision "
                + REVISION;
    }
     ...
     * 
     * @param config the servlet's configuration object @throws ServletException
     */
    public void init(ServletConfig config) throws ServletException {
        ...
        m_GraphVizPath = (m_GraphVizPath.startsWith("/") ? ""
                : (GraphVizRoot + "/"))
                + m_GraphVizPath;
        m_GraphVizFontDir = config.getInitParameter(GRAPH_VIZ_FONT_DIR);
        m_GraphVizFontDir = (m_GraphVizFontDir.startsWith("/") ? ""
                : (GraphVizRoot + "/"))
                + m_GraphVizFontDir;

        if (m_ServletTmpDir == null
                || (GraphVizRoot == null && (!m_GraphVizPath.startsWith("/") || !m_GraphVizFontDir
                        .startsWith("/")))) {
            System.err
                    .println("<html>"
     ...
     * 
     * @param req the request @param res the response @throws ServletException,
     * IOException
     */
    public void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        //  try {
        //    sRDF = java.net.URLDecoder.decode(sRDF);
        //    sURI = java.net.URLDecoder.decode(sURI);
        //  } catch (Exception e) {
     ...
     * 
     * @param req the request @param res the response @throws ServletException,
     * IOException, java.io.UnsupportedEncodingException
     */
     ...
     * 
     * @param out the servlet's output Writer @param r the Resource to output
     */
     ...
     * 
     * @param in the integer to convert and padd @param in the length of the
     * result @return the padded string
     */
     ...
     * 
     * @param out the servlet's output Writer @param l the Literal to output
     */
    static private void printNTripleLiteral(PrintWriter out, ALiteral l) {
            ...
            default:
                if (ar[i] >= 32 && ar[i] <= 127)
                    ...
                    if (ar[i] >= 0xDC00) {
                        ...
                        out
                                .print("{{{error: lone surrogate at end of string}}}");
                    } else if (ar[i] < 0xDC00 || ar[i] >= 0xE000) {
                        out
                                .print("{{{error: high surrogate not followed by low surrogate}}}");
                    }
        ...
        out.print("\" ");
        String s1 = "";
        if (l.getLang() != null && l.getLang().length() > 0) {//add language
                                                              // info if it
                                                              // exists
            s1 += "@" + l.getLang();
        }
     ...
     * 
     * @param out the servlet's output Writer @param subj the subject @param
     * pred the predicate @param objRes the object as a Resource (may be null)
    ...
    /*
     * Create a HTML anchor from the URI or anonNode of the given Resource
     * 
     * @param r the Resource @return the string as an HTML anchor
     */
     ...
     * @param out the servlet's output Writer @param subj the subject @param
     * pred the predicate @param objRes the object as a Resource (may be null)
     * @param objLit the object as a Literal (may be null) @param num the
     * statement number
     */
    static private void printTableRow(PrintWriter out, AResource subj,
        ...
        if (objRes != null)
            ...
            s1 = replaceString(s1, "&", "&amp;");
            s1 = "&quot;" + s1 + "&quot;";
            if (objLit.getLang() != null && objLit.getLang().length() > 0) {//add
                                                                            // language
                                                                            // info
                                                                            // if
                                                                            // it
            ...
            if (objLit.getDatatypeURI() != null) {//add datatype info if it
                                                  // exists
    ...
    private static class SH implements StatementHandler {
        ...
        /*
         * Constructuor for the StatementHandler. The primary responsiblitly is
         * to cache init variables
         * 
         * @param out the servlet's output Writer @param pw the Dot file's
         * PrintWriter syntax; otherwise use HTML syntax @param isNTriples if
         * true, output using the NTriples @param printTriples if true, print
         * the triples @param printGraph if true, create the graph file @param
         * printGraph if true, anonomyous nodes should be empty
         */
        public SH(PrintWriter out, PrintWriter pw, boolean isNTriples,
                boolean printTriples, boolean printGraph,
                boolean anonNodesEmpty, String graphFormat) {
            this.out = out;
            this.pw = pw;
         ...
         * 
         * @param subj the subject @param pred the predicate @param obj the
         * object (as a Resource)
         */
        public void statement(AResource subj, AResource pred, AResource obj) {
            if (!ARPServlet.AT_LEAST_ONE_TRIPLE) {
                ARPServlet.AT_LEAST_ONE_TRIPLE = true;
            }
            if (printTriples)
        ...
        /*
         * Handler for a Resource/Resource/Resource triple (S/P/O) Outputs the
         * given triple using NTriples or HTML syntax.
         * 
         * @param subj the subject @param pred the predicate @param obj the
         * object (as a Resource)
         */
        public void statementResource(AResource subj, AResource pred,
                AResource obj) {
            numStatements++;
        ...
        /*
         * Handler for a Resource/Resource/Literal triple (S/P/O) Outputs the
         * given triple using NTriples or HTML syntax.
         * 
         * @param subj the subject @param pred the predicate @param obj the
         * object (as a Literal)
         */
        public void statementLiteral(AResource subj, AResource pred,
                ALiteral lit) {
            numStatements++;
        ...
        /*
         * Print the first part of a triple's Dot file. See below for more info.
         * This is the same regardless if the triple's object is a Resource or a
         * Literal
         * 
        ...
        public void printFirstPart(AResource subj) {
            if (subj.isAnonymous()) {
                if (this.anonNodesEmpty) {
                    ...
                    if (n == null) {
                        this.numSubjects++;
                        subjects.put(subj.getAnonymousID(), new Integer(
                                this.numSubjects));
                        this.pw
                                .println("\"" + ANON_NODE
                                        + subj.getAnonymousID()
                ...
                if (gFormat != null && gFormat.equals(FORMAT_ISV_ZVTM)) {
                    this.pw.println("\"" + subj.getURI() + "\" [fontcolor=\""
                            + Float.toString(ARPServlet.resTBh) + ","
                            + Float.toString(ARPServlet.resTBs) + ","
                            + Float.toString(ARPServlet.resTBv) + "\",URL=\""
                            + subj.getURI() + "\"];");
                } else {
        ...
        /*
         * Handler for a Resource/Resource/Resource triple (S/P/O). Outputs the
         * given triple using Dot syntax.
         * 
         * Each triple will be output in three lines of DOT code as follows (not
         * including the complication of anon nodes and the possiblity that the
         * anon nodes may be named with an empty string):
         * 
         * 1. " <subject>" [URL=" <subject">]; 2. " <subject>" -> " <object>"
         * [label=" <predicate>",URL=" <predicate>"]; 3. " <object>" [URL="
         * <object>"];
         * 
         * @param subj the subject @param pred the predicate @param obj the
         * object (as a Resource)
         */
        public void statementDotResource(AResource subj, AResource pred,
                AResource obj) {
            if (this.pw == null)
                return;
            ...
            if (obj.isAnonymous()) {
                if (this.anonNodesEmpty) {
                    if (gFormat != null && gFormat.equals(FORMAT_ISV_ZVTM)) {
                        this.pw.println("\"" + ANON_NODE + obj.getAnonymousID()
                                + "\" [fontcolor=\""
                                + Float.toString(ARPServlet.prpTh) + ","
                                + Float.toString(ARPServlet.prpTs) + ","
                                + Float.toString(ARPServlet.prpTv)
                ...
                if (gFormat != null && gFormat.equals(FORMAT_ISV_ZVTM)) {
                    this.pw.println("\"" + obj.getURI() + "\" [fontcolor=\""
                            + Float.toString(ARPServlet.prpTh) + ","
                            + Float.toString(ARPServlet.prpTs) + ","
                            + Float.toString(ARPServlet.prpTv) + "\",label=\""
                            + pred.getURI() + "\",URL=\"" + pred.getURI()
                            + "\"];");
                    this.pw.println("\"" + obj.getURI() + "\" [fontcolor=\""
                            + Float.toString(resTBh) + ","
                            + Float.toString(resTBs) + ","
                            + Float.toString(resTBv) + "\",URL=\""
                            + obj.getURI() + "\"];");
                } else {
         ...
         * anon nodes may be named with an empty string):
         * 
         * 1. " <subject>" [URL=" <subject">]; 2. " <subject>" -> " <literal>"
         * [label=" <predicate>",URL=" <predicate>"]; 3. " <literal>"
         * [shape="box"];
         * 
         * @param subj the subject @param pred the predicate @param obj the
         * object (as a Literal)
         */
        public void statementDotLiteral(AResource subj, AResource pred,
                ALiteral lit) {
            if (this.pw == null)
                return;
             ...
             * so they must be replaced with spaces o Limit the number of chars
             * to make the graph legible o Escape double quotes
             */
            ...
            if (gFormat != null && gFormat.equals(FORMAT_ISV_ZVTM)) {
                this.pw.println("\" [fontcolor=\""
                        + Float.toString(ARPServlet.prpTh) + ","
                        + Float.toString(ARPServlet.prpTs) + ","
                        + Float.toString(ARPServlet.prpTv) + "\",label=\""
                        + pred.getURI() + "\",URL=\"" + pred.getURI() + "\"];");
                this.pw.println("\"" + tmpName + "\" [fontcolor=\""
                        + Float.toString(litTBh) + "," + Float.toString(litTBs)
                        + "," + Float.toString(litTBv)
                        + "\",shape=box,label=\"" + tmpObject + "\"];");
            } else {
                this.pw.println("\" [label=\"" + pred.getURI() + "\",URL=\""
                        + pred.getURI() + "\"];");
                this.pw.println("\"" + tmpName + "\" [shape=box,label=\""
                        + tmpObject + "\"];");
            }
        }
    }
        ...
        try {
            ...
            if (c) {
                if (AT_LEAST_ONE_TRIPLE) {
                    out
                            .println("<p>Your RDF document validated successfully.</p>");
                } else {
                    out
                            .println("<p>Error: Your document does not contain any RDF statement.</p>");
                }
                AT_LEAST_ONE_TRIPLE = false;
            }

            /*
             * the following should not happen anymore, as we use ARP2 which
             * supports datatypes, but leave it there for now, just in case
             */
            s = eh.getDatatypeErrors();
            if (s != null && s.length() >= 2) {//2 to prevent an
                                               // arrayindexoutofbounds
                                               // exception
                out.println("<h3>Note about datatypes</h3>");
                out
                        .println("<p>Datatypes are used on lines "
                                + s.substring(0, s.length() - 2)
                                + ". This RDF feature is not yet supported by the RDF Validator. Literals are treated as untyped.</p>");
            }
    ...
    /*
     * Initialize the graph output file. If an error occurs, this function will
     * print an error message.
     * 
     * @param out the servlet's output Writer @req the servlet request object
     * @return the File object for the graph file; null if an error occurs
     */
    private File initGraphFile(PrintWriter out, HttpServletRequest req) {
        try {
            // Stop if any of the parameters are missing
            if (m_ServletTmpDir == null || m_GraphVizPath == null
                ...
                out.println("Please send a message to <a href='mailto:"
                        + MAIL_TO + "'>" + MAIL_TO
                        + "</a> and mention this problem.");
                return null;
            }
        } catch (Exception e) {
            System.err
                    .println("Unable to create a temporary graph file. A graph cannot be generated.");
            return null;
        }
        ...
        if (dotFile == null) {
            out
                    .println("<h1>Failed to create a temporary graph file. A graph cannot be generated.</h1>");
            return null;
        }
     ...
     * 
     * @param out the servlet's output Writer @param uri the URI to check
     * @return true if the URI is supported; false otherwise
     */
    private boolean isURISupported(PrintWriter out, String uri) {
        try {
            if (uri.length() >= 4
                    && uri.substring(0, 4).equalsIgnoreCase("file")) {
                out.println("<h1>file URI Schemes are NOT Supported</h1>");
                out
                        .println("URIs from the 'file' URI scheme are not supported by this servlet.");
                return false;
            }
        } catch (Exception e) {
    ...
    private void process(HttpServletRequest req, HttpServletResponse res,
        ...
        if (!sURI.equals("")) {
            ...
            try {
                ...
                if (sRDF == null)
                    throw new getRDFException(
                            "The URI may not exist or the server is down.@@");
        ...
        if (printGraph) {
            ...
            if (pw != null)
                ...
                processGraphParameters(req, pw, (sFormat
                        .equals(FORMAT_PNG_EMBED))
                        || (sFormat.equals(FORMAT_PNG_LINK))
                        || (sFormat.equals(FORMAT_GIF_EMBED))
                        || (sFormat.equals(FORMAT_GIF_LINK)));
        }

        // Create the StatementHandler - it will handle triples for
        // the table/ntriples and the graph file
        SH sh = new SH(outtmp, pw, nTriples, printTriples, printGraph,
                anonNodesEmpty, sFormat);
        ...
        if (sFormat != null && printGraph) {
            generateGraph(out, pw, dotFile, sRDF, req, sFormat,
                    (sSaveRDF != null) ? true : false, (sSaveDOTFile != null
                            && sSaveDOTFile.equals("on") ? true : false));
        }
